...
if __name__ == '__main__':
    ...
    if os.path.isfile('unstable_configurations_info_pretest.pkl'):
        # Load configurations
        with open('unstable_configurations_info_pretest.pkl', 'rb') as f:
    ...
    while 1:
        ...
        config_info['position'] = list(config_info['position'])
        config_info['boxidx'] = list(config_info['boxidx'])
        config_info['pos_xmax'] = list(config_info['pos_xmax'])
        config_info['pos_ymax'] = list(config_info['pos_ymax'])
        config_info['overlap_xthr'] = list(config_info['overlap_xthr'])
        config_info['overlap_ythr'] = list(config_info['overlap_ythr'])
        ...
        # Find the lowest box(es)
        low_boxid = np.where(boxpos[:,-1] == boxpos[:,-1].min())[0]
        # Get their size
        box_size_low = box_size[boxidx[low_boxid],:]
        # Get the lowest box with its bottom surface is most close to the floor
        lowest_boxid = low_boxid[box_size_low[:,-1].argmin()]
        # Evaluation
            # bottom surface position height
        bsp_height = boxpos[lowest_boxid, :][-1]
            # bottom surface position height in theory
        bsp_height_theory = box_size[:, boxidx[lowest_boxid]][-1]/2
        # print('min surface is {}, min surface in theory is {}'.format(bsp_height, bsp_height_theory))
        if bsp_height != bsp_height_theory+ (0.1/2):
            print('Not suitable')
            continue
        ...
        isstable_all = stability_func.examine_stability(box_pos_ori_all, box_pos_fin_all, tol=0.05)
        ...
        # Human judgment
        if (not isstable) and (record_flag):
        ...
        if iter_time % 5 == 0:
            config_info['position'] = np.array(config_info['position'])
            config_info['boxidx'] = np.array(config_info['boxidx'])      
            config_info['pos_xmax'] = np.array(config_info['pos_xmax'])
            config_info['pox_ymax'] = np.array(config_info['pos_ymax'])
            config_info['overlap_xthr'] = np.array(config_info['overlap_xthr'])
            config_info['overlap_ythr'] = np.array(config_info['overlap_ythr'])

            # Temporary store
            with open('unstable_configurations_info_pretest.pkl', 'wb') as f:
